...
import { type QueuedTrack, useAIQueueStore } from "../../store";
...
async function searchAndGetSpotifyUri(
  ...
  try {
    ...
    for (const track of results) {
      ...
      if (
        trackArtists.includes(artistLower) ||
        artistLower.includes(trackArtists.split(",")[0])
      ) {
        if (
          trackTitle.includes(trackLower) ||
          trackLower.includes(trackTitle)
        ) {
...
Suggest 5 tracks that would flow well. Consider energy, mood, and genre continuity.`;
    ...
    for (const suggestion of shuffledSuggestions) {
      const uri = await searchAndGetUri(
        suggestion.n,
        suggestion.a,
        musicProviderType,
      );
...
export async function startAIQueue(mood?: string): Promise<void> {
  ...
  try {
    ...
    const message =
      err instanceof Error ? err.message : "Failed to start AI Queue";
    ...
    try {
      ...
      if (musicProviderType === "spotify") {
        ...
        if (lastTrackUri && store.queue.length > 0 && playbackState) {
          const lastIndex = store.queue.findIndex(
            (t) => t.uri === lastTrackUri,
          );
          if (lastIndex !== -1) {
            ...
            if (
              durationMs > 0 &&
              progressMs >= durationMs - endThreshold &&
              !isPlaying
            ) {
